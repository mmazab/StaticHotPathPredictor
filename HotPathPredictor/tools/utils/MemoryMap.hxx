...
//#define DEBUG_ADDRESS debug_address

namespace Memory {

    extern uint64_t debug_address;

    extern FILE *debug_log;

    void memory_map_init(FILE *debug);

    bool filter_address(uint64_t addr);

    bool check_address_in_range(uint64_t addr, uint64_t base, uint64_t length);
	...
	read_track_t valid[TRACK_SIZE];
        ...
        read_track_t invalid[TRACK_SIZE];
	...
	uint8_t is_offset_valid(const uint32_t offset, uint8_t length) const {
        ...
        uint8_t is_offset_invalid(const uint32_t offset, uint8_t length) const {
	    const uint32_t byte_offset = offset >> TRACK_SHIFT_OFFSET;
	    const uint32_t bit_offset = offset % TRACK_BITS_PER_INDEX;
	    const read_track_t omask = offsetMask(length, bit_offset);
	    const read_track_t bits = (this->invalid[byte_offset] & omask);
	    return (bits == omask);
	}
	...
	void set_offset_valid(const uint32_t offset, uint8_t length) {
	    const uint32_t byte_offset = offset >> TRACK_SHIFT_OFFSET;
	    const uint32_t bit_offset = offset % TRACK_BITS_PER_INDEX;
	    const read_track_t valid_mask = offsetMask(length, bit_offset);

#ifdef DEBUG_ADDRESS
            if (check_address_in_range(0x2000000002c466c0, (this->getAddress() + offset), length)) {
                fprintf(debug_log, "Setting valid %d bytes at %p\n", length, this->getAddress() + offset);
                print_backtrace(debug_log, 10);
            }
#endif

	    this->valid[byte_offset] |= valid_mask;
            this->invalid[byte_offset] &= ~valid_mask;
	}
	
	void set_offset_invalid(const uint32_t offset, uint8_t length) {
	    const uint32_t byte_offset = offset >> TRACK_SHIFT_OFFSET;
	    const uint32_t bit_offset = offset % TRACK_BITS_PER_INDEX;
	    const read_track_t invalid_mask = offsetMask(length, bit_offset);

#ifdef DEBUG_ADDRESS
            if (check_address_in_range(0x2000000002c466c0, (this->getAddress() + offset), length)) {
                fprintf(debug_log, "Setting invalid %d bytes at %p\n", length, this->getAddress() + offset);
                print_backtrace(debug_log, 10);
            }
#endif

            this->invalid[byte_offset] |= invalid_mask;
	    this->valid[byte_offset] &= ~invalid_mask;
	}

	void set_offset_unknown(const uint32_t offset, uint8_t length) {
	    const uint32_t byte_offset = offset >> TRACK_SHIFT_OFFSET;
	    const uint32_t bit_offset = offset % TRACK_BITS_PER_INDEX;
	    const read_track_t mask = offsetMask(length, bit_offset);
	    this->valid[byte_offset] &= ~mask;
            this->invalid[byte_offset] &= ~mask;
	}
    ...
    public:
	    ...
	    memset(this->invalid, 0, sizeof(this->invalid));
	}
	...
	void clear() {
	    memset(this->valid, 0, sizeof(this->valid));
            memst(this->invalid, 0, sizeof(this->invalid));
	}
	...
	uint8_t is_invalid(const void *addr, uint8_t length) const {
	    check_range(addr, length);
	    const uint32_t offset = am_offset(addr);
	    return this->is_offset_invalid(offset, length);
	}
	...
	void set_invalid(const void *addr, uint8_t length) {
	    ...
	    const uint32_t offset = am_offset(addr);
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (this->is_offset_valid(coff, 1)) {
                    this->set_offset_unknown(coff, 1);
                } else {
                    this->set_offset_invalid(coff, 1);
                }
            }
	}

	const valid_t get_aligned_validity(const void * addr, uint8_t length) const {
    ...
    protected:
        typedef hash_map<pageaddr_t, T *, PageAddrHash, PageAddrEquals> PageMap;
	...
	bool is_valid(const void *addr) const {
	    ...
	    return (node->template is_valid(addr, sizeof(S)));
	}

	template <class S>
	void set_invalid(const void *addr) {
	    T *node = this->get_or_create_node(addr);
	    node->template set_invalid(addr, sizeof(S));
	}

	template <class S>
	void set_valid(const void *addr) {
	    T *node = this->get_or_create_node(addr);
	    node->template set_valid(addr, sizeof(S));
	}
    };
	...
	template <class S>
	const valid_t get_aligned_validity(const void * addr) const {
	...
	void write_value(const void *addr, const T & value) {
	    ...
	    const uint32_t offset = this->am_offset(addr);
            ...
            if (this->is_offset_invalid(offset, sizeof(T))) {
                fprintf(stderr, "Attempt to write %d byte value over invalidated address %p\n", sizeof(T), addr);
                abort();
            }
	    ...
	    for (uint64_t i = 0; i < sizeof(this->valid); i++) {
                ...
                for (uint64_t granule = offset; granule < offset + MemoryPage<uint8_t, PAGE_BITS>::TRACK_BITS_PER_INDEX ; granule++) {
                    if (node->is_offset_valid(granule, 1)) {
                        ...
                        this->set_offset_valid(granule, 1);
                        live_bytes++;
                    } else if (node->is_offset_invalid(granule, 1)) {
                        if (this->is_offset_invalid(granule, 1)) {
                            this->set_offset_unknown(granule, 1);
                        } else {
                            this->set_offset_invalid(granule, 1);
                        }
                    }
                }
            }
	...
	void merge_old(const BytePage *node) {
            uint64_t live_bytes = 0;
...
//            uint64_t live_bytes = 0;
		...
		this->valid[i] |= validity;
                ...
                if (validity == 0) {
                    ...
                    for (int j = 0; j < ((sizeof(validity) * 8) / 4); j++) {
                        switch(validity & 0xf) {
                        case 0:
                            break;
                        case 1:
            ...
            for (uint64_t granule = 0; granule < this->PAGE_SIZE; granule++) {
                if (this->is_offset_valid(granule, 1)) {
                    uint8_t *maddr = (uint8_t *) (intptr_t) (addr | granule);
                        ...
                        {
                            fprintf(fp, "Committing (%lx | %lx) = %p [%" PRIx8 "]\n", addr, granule, maddr, this->values[granule]);
                        }
#endif
	...
	uint32_t print_valid_ranges(FILE *fp) const {
	    ...
	    for (uint64_t granule = 0; granule < this->PAGE_SIZE; granule++) {
            ...
            for (uint64_t granule = 0; granule < this->PAGE_SIZE; granule++) {
                while ((granule < this->PAGE_SIZE) && !this->is_offset_invalid(granule, 1)) {
                    granule++;
                }
	...
	T read_aligned_value(const void * addr) const {
	    ...
	    return node->template read_value<T>(addr); //ajamshid 9/4/13 added "template "
	}
	...
	void write_aligned_value(void * addr, const T &value) {
	    PageType::check_addr_range(addr, sizeof(T)); 
	    PageType *node = this->get_or_create_node(addr);
	    node->template write_value<T>(addr, value); //ajamshid 9/4/13 added "template "
	}
	...
	T read_value_committed(const void * addr) const {
	    ...
	    return node->template read_value_committed<T>(addr); //ajamshid 9/4/13 added "template "
	}
